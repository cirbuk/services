...
export default (
  ...
  req,
  uri,
  {verbose = false, timestamp = false, prefix: prefixFn}
) => {
  ...
  return {
    ...
    response(res) {
      ...
      logger.info(
        '%s %s %s %s %s %s',
        prefixStr,
        chalk.magenta(protocol(uri)),
        chalk.cyan(method(req)),
        st,
        path(req),
        `${chalk.gray('(')}${chalk[colorForSpeed(elapsed)](
          `${elapsed}ms`
        )}${chalk.gray(')')}`
      );
      const hasErred = res.status >= 400;
      if (verbose || hasErred) {
        const colorFn = hasErred
          ? chalk.red.bind(chalk)
          : chalk.gray.bind(chalk);
