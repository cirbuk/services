...
export class Manager {
  ...
  constructor({ref, config, ...options} = {}) {
    if (isUndefined(ref) && !isPlainObject(config)) {
      throw new Error(
        `Either "ref" or "config"(service configuration) should be provided`
      );
    ...
    const serviceDef = resolver.resolve(config, this.initData, {
      ...
      mappers: [
        [
          /\[\[(.+?)]]/,
          (match, formula) =>
            this.transformers[formula] || this.transformers.default,
        ],
      ],
    });
      ...
      return new Promise((resolve, reject) => {
        const handler = (snap) => {
          ...
          if (!isNull(data)) {
            ...
            reject(
              new Error(
                'No firebase service configuration found to initialize services.'
              )
            );
        ...
        this.servicesRef.on('value', handler);
      });
    }
    this.setServices();
